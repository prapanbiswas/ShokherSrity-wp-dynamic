...
<main>
    ...
    <?php $hero_imgs = ss_get_hero(); $hero_desktop = esc_url($hero_imgs['desktop']); $hero_mobile = esc_url($hero_imgs['mobile']); ?>
    <!-- Hero Section -->
    <section class="hero" data-aos="fade" style="--hero-bg-image: url('<?php echo $hero_desktop; ?>')">
...
(function(){
    var desktop = <?php echo wp_json_encode($hero_desktop); ?>;
    var mobile  = <?php echo wp_json_encode($hero_mobile); ?>;
    function swap(isDesktop) {
        var h = document.querySelector('.hero');
        if (h) h.style.setProperty('--hero-bg-image', "url('" + (isDesktop ? desktop : mobile) + "')");
    }
    var mq = window.matchMedia('(min-width: 769px)');
    swap(mq.matches);
    mq.addEventListener('change', function(e){ swap(e.matches); });
    // Also run on DOM ready in case .hero isn't in DOM yet
    document.addEventListener('DOMContentLoaded', function(){ swap(mq.matches); });
})();
</script>
